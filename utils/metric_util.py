...
#         self.hist_list = self.hist_list[-100:]  # 只保留最近100次迭代的结果
#
#     def compute(self):
#         if not self.hist_list:
#             return 0, self.best_miou  # 如果没有数据，返回0和当前的best_miou
#         iou = np.nan_to_num(per_class_iu(sum(self.hist_list)))
#         mean_iou = np.nanmean(iou)
#         if mean_iou > self.best_miou:
#             self.best_miou = mean_iou
#         self.hist_list = []
#         return mean_iou, self.best_miou
